...
适配器按两种策略处理 stderr 数据：

| 策略 | 格式 | 适用适配器 |
|------|------|------------|
| JSON 包装 | `{ type: 'error', content: msg }` + `\n` | Claude Code, Gemini, Copilot, Cursor, Cline, Continue, Amp, Qwen |
| 纯文本前缀 | `Error: <msg>\n` | Aider, Amazon Q, Goose |
| JSON (status) | `{ type: 'status', content: msg, source: 'stderr' }` + `\n` | Codex |

JSON 包装策略的适配器将 stderr 作为结构化 JSON 行注入流中，由对应的 JSON Parser 解析。纯文本前缀策略的适配器直接在 stderr 内容前加 `Error: ` 前缀注入流中，由 TextStreamParser 的错误模式检测（`/^Error:/i`）识别。Codex 比较特殊，将 stderr 标记为 `status` 类型而非 `error`。
...
- **版本检测特殊性**：`isInstalled()` 和 `getVersion()` 使用 `q version`（无 `--` 前缀，与其他工具不同）
- **环境变量前缀**：`AWS_`、`AMAZON_`
...
- **特殊设计**：`-y` 既是执行命令的一部分也兼作 yolo 标志（自动确认）；`--json` 在 `permissionMode === 'skip'` 时才传递
...
- **buildArgs 不接受 opts**：`buildArgs(prompt)` 签名不接受 `ExecOptions`，因为 Goose 无 CLI 级别的 yolo 标志
