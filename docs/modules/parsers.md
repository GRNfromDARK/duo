...
1. **策略一：code-fenced JSON 代码块** — 使用正则 `` /```json\s*\n([\s\S]*?)```/gi ``（大小写不敏感）全局匹配，取最后一个匹配结果。选择"最后一个"是因为 God LLM 可能在推理过程中输出多个 JSON 块，最终结果通常在最后。
2. **策略二：裸 JSON 对象** — 找到文本中第一个 `{` 和最后一个 `}` 之间的内容尝试解析。这是 BUG-23 的修复，处理 God LLM 输出 JSON 但不加代码围栏的情况。

**返回值语义**：
- `null` — 两种策略均未找到 JSON（纯文本输出）
...
**重试策略**（BUG-23 修复后的行为——永远不返回 `null`，始终返回 `ExtractResult` 并携带具体错误信息）：

- 纯文本输出（无 JSON 块）-> 返回 `{ success: false, error: '...' }`，包含输出长度等诊断信息，**不重试**
- 首次提取成功 -> 返回结果（附带 `sourceOutput` 字段记录原始输出）
- 首次提取失败（JSON 存在但校验不通过）-> 调用 `retryFn(error)` 获取新输出 -> 再次提取
- 重试后无 JSON -> 返回包含原始错误和重试错误的组合错误信息
- 重试后校验仍失败 -> 返回包含原始错误和重试校验错误的组合错误信息
...
| 输出 | `AsyncIterable<OutputChunk>` | `ExtractResult<T>` |
