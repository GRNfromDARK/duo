...
2. 在 `App.tsx` 的 EXECUTING 状态 hand execution callback 中（约第 1394 行），当 God 决策的 side effects 被应用到编排状态时，填充 `lastUnresolvedIssuesRef`。
   ...
   **精确定位**：在 `App.tsx` 第 1394-1422 行的 hand execution callback 中，`pendingCoderMessage` 被写入 `pendingInstructionRef` 之后（第 1396 行），phase transition 清空 `lastUnresolvedIssuesRef` 之前（第 1409 行）。在此处插入：
   ...
   // 注意：这里的 ctx 是 xstate machine snapshot context（通过 state.context 获取），
   // 不是 handContext。lastReviewerOutput 存储在 workflow machine context 上。
   ...
   **注意**：此代码位于 EXECUTING 阶段（hand executor 回调），不是 GOD_DECIDING 阶段。God 决策在 GOD_DECIDING 阶段生成，但 side effects（包括 `pendingCoderMessage`、phase transitions）在 EXECUTING 阶段的 hand executor 回调中被应用到编排状态。`lastReviewerOutput` 存在于 xstate workflow machine context 上（非 handContext）。
...
> **注意**：Coder adapter_unavailable 后重试的场景在 Change 1 设计下 `isPostReviewerRouting = false`（因为 `lastWorkerRoleRef` 已经是 `'coder'`）。这是 Change 5 要修复的 gap——参见 Change 5 中的完整行为矩阵。
...
if (matchesAny(raw, AUTH_FAILED_PATTERNS)) {
  ...
  // 复用 god-decision-service.ts 中的 stripToolMarkers() 函数，避免维护平行 regex。
...
**注意**：需要从 `god-decision-service.ts` 导入 `stripToolMarkers`（Change 1 也需要此函数，可统一导入或提取到共享 utils）。
...
**设计取舍**：
- **中文**保持宽松匹配——中文的实现/开发/编写/修改几乎总是明确的实现意图，误匹配概率低（不像英文 "fix the gap" 中 "fix" 有多义性）。添加可选前缀 `请/去/要` 扩大覆盖。
- **英文**收窄为短语级——"fix the gap"、"code discovery" 不再匹配，但 "implement the fix"、"fix the code" 仍然匹配。
- **替代方案（更保守）**：完全移除自动覆盖，改为让 God 在 `send_to_coder` 时显式指定 `effectiveType`。但这需要修改 GodDecisionEnvelope schema，变更范围更大。
...
- [ ] **删除** `audit-bug-regressions.test.ts` 第 496-529 行的 2 个测试：`test_regression_r2_bug1_post_coder_actions_match_schema`（断言 `continue_to_review`/`retry_coder`）和 `test_regression_r2_bug1_post_reviewer_actions_match_schema`（断言 `route_to_coder`/`converged` 等），这些 action names 不再出现在 prompt 中
- [ ] **删除或改写** `bug-15-16-17-18-regression.test.ts` 第 437-446 行的测试：`god system prompt mentions god_override system_log constraint`——更新后的 `buildGodSystemPrompt()` 移除了 Rule 6（`god_override`/`system_log` 约束），该测试**将会失败**。可改写为验证新 prompt 包含 `TASK_INIT` 格式和基本 Rules
- [ ] 现有（更新后的）测试全部通过
...
| `src/__tests__/engine/bug-15-16-17-18-regression.test.ts` | 修改 | 第 437-446 行的测试（`god system prompt mentions god_override system_log constraint`）**将会失败**——更新后的 `buildGodSystemPrompt()` 移除了 Rule 6（含 `god_override`/`system_log`），需删除或改写为验证新 prompt 结构 |
